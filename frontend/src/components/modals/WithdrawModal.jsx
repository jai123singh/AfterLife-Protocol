...
const WithdrawModal = ({
  ...
  useEffect(() => {
    if (isConfirmed) {
      toast.success("Withdrawal successful. Funds have been withdrawn.", {
        id: "withdraw",
        position: "top-center",
      });
      setAmount("");
  ...
  useEffect(() => {
    if (writeError || waitError) {
      ...
      if (mainError) {
        toast.error(mainError, { id: "withdraw", position: "top-center" });
      }
      setIsSubmitting(false);
      setIsProcessing(false);
      reset();
  ...
  const validateAmount = (value) => {
    ...
    // Check if exceeds total deposit
    const maxAmount = formatEther(totalDeposit || 0n);
    if (parsedAmount > parseFloat(maxAmount)) {
      setError("Amount exceeds your total deposit");
      return false;
    }
        ...
        <div className="p-4 bg-background/40 rounded-xl space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">
              {formatEther(totalDeposit || 0n)} ETH
            </span>
          </div>
          {estimatedGas && (
            <div className="flex justify-between items-center">
